...
contract Maximizer is IMaximizer, Ownable2Step, Pausable, EIP712, Nonces {
        ...
        EIP712("Maximizer", "1")
    {
        ...
        for(uint256 i; i < length; ++i){
            if (_allowedTokens[i] == address(0)) revert TokenCannotBeZeroAddress();
    ...
    function depositETHFor(address _for) whenNotPaused payable external {
        ...
        if (_for== address(0)) revert CannotDepositForZeroAddress();
        if (!allowedTokens[WETH_ADDRESS]) revert TokenNotAllowedForMaximizeYields();
    ...
    function migrate(
        ...
        if (!SignatureChecker.isValidSignatureNow(secureSigner, constructedHash, _authorizationSignatureFromZircuit)){
            revert SignatureInvalid();
        }
    ...
    function setStakable(address _token, bool _canStake) external onlyOwner {
        ...
        if (allowedTokens[_token] == _canStake) revert TokenAlreadyConfiguredWithState();
    ...
    function blockMigrator(address _migrator, bool _blocklisted) external onlyOwner {
        ...
        if (blockedMigrator[_migrator] == _blocklisted) revert MigratorAlreadyAllowedOrBlocked();
